...
def mydb():
    db = db_util.DB("serialnumbers")
    yield db
    db.delete_from_db('extended_warranty', 'order_id', 1234)
    db.delete_from_db('serial_numbers', 'order_id', 1234)
    db.delete_from_db('orders', 'so_id', 1234)
    db.close_db()
...
def test_find(mydb):
    ...
    assert mydb.find("orders.so_number", 'so-123') == find
    assert mydb.find('orders.shipping_name', 'testorder ship') == find
    assert mydb.find('orders.billing_name', 'testorder bill') == find
    assert mydb.find('orders.billing_postcode', 'bb1234') == find
    assert mydb.find('orders.shipping_postcode', 'ss1234') == find
    assert mydb.find('serial_numbers.sn', '67890') == find
    #partial match
    assert mydb.find('orders.shipping_name', 'testorder') == find
...
def test_delete_from_db(mydb):
    ...
    mydb.insert_order(order)
    mydb.delete_from_db('orders', 'so_number', 'so-to_delete')
...
def test_update_postcode(mydb):
    ...
    assert mydb.find('orders.shipping_postcode', 'SS1234 updated') == [(1234, 'so-123', datetime.date(2020, 10, 12), datetime.date(2020, 10, 13), 2)]
...
def test_update_addresses(mydb):
    ...
    assert mydb.find('orders.billing_name', 'updated billing') == [(1234, 'so-123', datetime.date(2020, 10, 12), datetime.date(2020, 10, 13), 2)]
