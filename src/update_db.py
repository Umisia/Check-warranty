...
def add_data(data, db):
    ...
    log.info(order_data)
    db.insert_order(order_data)# add order to db, table:orders # needs values in tuple

    if data.serials:# add serial numbers to db, table:serial_numbers
        try:
            db.insert_serials(data.so_id, data.serials)  # so_id:int, serials:list
        except:
            db.delete_from_db("orders", "so_id", data.so_id)  # undo order insertion, so the order can be catched again as a new one
...
if __name__ == '__main__':
    ...
    if args.populate:
        log.info("populate")
        pages = 50
        do_crm_orders(pages, mydb)
        do_inv_orders(pages, mydb)
    else:
        log.info("update")
        pages = 1
        do_inv_orders(pages, mydb)
