...
def upload():
    ...
    for row in range(2, lastrow + 1):
        ...
        if so_id.isdigit():
            ...
            log.info(f"writing warranty {so_id}, {warranty}, from row:{row},col:{lastcol}")
...
def find_orders():
    ...
    if len(input_val) >= 4:
        ...
        log.debug(f"search_finds: {search_finds}")
...
canvas = tkinter.Canvas(frame, width=220, highlightthickness=0)
